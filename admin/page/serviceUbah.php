         ...
         <div class="box box-primary">
            ...
            if(isset($_POST['aksi'])){
                ...
                if($_FILES['d']['error'] > 0 && $_FILES['c']['error'] > 0){
                    $link->query("UPDATE services set title='$a',description='$b' where id_service='$id'");
                    ...
                    $link->query("UPDATE services set title='$a',description='$b',pict='$c',pict_hover='$d' where id_service='$id'");
                }
                else if($_FILES['c']['error'] < 0){
                    $c = date('sYdhmi').$_FILES['c']['name'];
                    move_uploaded_file($_FILES['c']['tmp_name'],"../images/services/".$c);
                    $link->query("UPDATE services set title='$a',description='$b',pict='$c' where id_service='$id'");
                }
                else if($_FILES['c']['error'] < 1 && $_FILES['d']['error'] < 0){
                    $d = date('sYdhmi').$_FILES['d']['name'];
                    move_uploaded_file($_FILES['d']['tmp_name'],"../images/services/".$d);
                    $link->query("UPDATE services set title='$a',description='$b',pict_hover='$d' where id_service='$id'");
                }
            ?>
            <div class="callout callout-info">
             ...
             <?php } ?>
             <?php 
              $data = $link->query("SELECT * from services where id_service='$id'")->fetch_assoc();
             ?>
             </div>
